...
#include <gazebo_plugins/gazebo_ros_vacuum_gripper_modified.h>
...
void GazeboRosVacuumGripper::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  ROS_INFO_NAMED("vacuum_gripper", "modified_Loaded gazebo_ros_vacuum_gripper");
}
...
{
  if (status_) {
    ROS_WARN_NAMED("vacuum_gripper", "modified_gazebo_ros_vacuum_gripper: already status is 'on'");
  } else {
    status_ = true;
    ROS_INFO_NAMED("vacuum_gripper", "modified_gazebo_ros_vacuum_gripper: status: off -> on");
  }
  return true;
}
                                     ...
                                     std_srvs::Empty::Response &res)
{
  if (status_) {
    status_ = false;
    ROS_INFO_NAMED("vacuum_gripper", "modified_gazebo_ros_vacuum_gripper: status: on -> off");
  } else {
    ROS_WARN_NAMED("vacuum_gripper", "modified_gazebo_ros_vacuum_gripper: already status is 'off'");
  }
  return true;
}
...
void GazeboRosVacuumGripper::UpdateChild()
{
  ...
  if(sucked == 0){
    //ROS_INFO_NAMED("vacuum_gripper", "link_name:%d",GAZEBO_MAJOR_VERSION);
  //ROS_INFO_NAMED("vacuum_gripper", "link_name:%s,parent_name:%s.",link_->GetName().c_str(),parent_->GetName().c_str());
  for (size_t i = 0; i < models.size(); i++) {
    //ROS_INFO_NAMED("vacuum_gripper", "model_name:%s",models[i]->GetName().c_str());
  ...
  if(sucked == 1){
    #if GAZEBO_MAJOR_VERSION >= 8
          ignition::math::Pose3d link_pose = link_sucked->WorldPose();
    #else
          ignition::math::Pose3d link_pose = link_sucked->GetWorldPose().Ign();
    #endif
    #if GAZEBO_MAJOR_VERSION >= 8
            link_sucked->SetLinearVel(link_->WorldLinearVel());
            link_sucked->SetAngularVel(link_->WorldAngularVel());
    #else
            link_sucked->SetLinearVel(link_->GetWorldLinearVel());
            link_sucked->SetAngularVel(link_->GetWorldAngularVel());
    #endif
    link_pose.Set(parent_pose.Pos(), link_pose.Rot());
    link_sucked->SetWorldPose(link_pose);
  }
#ifdef ENABLE_PROFILER
  IGN_PROFILE_END();
